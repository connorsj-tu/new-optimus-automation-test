...
select top 100 * from [CCRMain].[dbo].PersonAlias 
order by AliasId desc

    SET IDENTITY_INSERT [CCRMain].[dbo].PersonAlias ON
insert into [CCRMain].[dbo].PersonAlias (AliasId, PersonId, PersonLinkId, Deleted, UProcId, ULoadId, CLoadId, Disalias)
values (15352010, 9999, 9998, 0, 500300, 500300, 500300, 0)
 SET IDENTITY_INSERT [CCRMain].[dbo].PersonAlias OFF
 commit; 


 select max(SourceId) from  [CCRMain].[dbo].PersonAliasSource

 select top 100 * from [CCRMain].[dbo].PersonAliasSource
order by SourceId desc

    SET IDENTITY_INSERT [CCRMain].[dbo].PersonAliasSource ON
insert into [CCRMain].[dbo].PersonAliasSource (AliasId, CLoadId, CSrcId, BLoadId, BSrcId, UProcId, Implied, Deleted, DataOrigin, SourceId, ULoadId,Disalias)

values(99999, 500300, 1, 500, 1, 500300, 0, 0, 1, 25189213, 500300, 0)
 SET IDENTITY_INSERT [CCRMain].[dbo].PersonAliasSource OFF
 commit; 
